...
* **Will enabling / using this feature result in any new API calls?**
  Kube-Proxy will include a Node informer that will watch the node it is running
  on when this feature is enabled. This is also the case for a couple other
  Kube-Proxy features, including the previous `ServiceTopology` feature gate.
  This may result in some additional calls to the EndpointSlice API, but the
  expected increase is minimal.
